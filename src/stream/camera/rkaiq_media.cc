...
void RKAiqMedia::GetCifDvpSubDevs(int id, struct media_device *device,
  ...
  entity = media_get_entity_by_name_rk(device, "stream_cif_dvp_id1");
  if (entity) {
    entity_name = media_entity_get_devname(entity);
    if (entity_name) {
  ...
  entity = media_get_entity_by_name_rk(device, "stream_cif_dvp_id2");
  if (entity) {
    entity_name = media_entity_get_devname(entity);
    if (entity_name) {
  ...
  entity = media_get_entity_by_name_rk(device, "stream_cif_dvp_id3");
  if (entity) {
    entity_name = media_entity_get_devname(entity);
    if (entity_name) {
  ...
  entity = media_get_entity_by_name_rk(device, "rkcif-dvp");
  if (entity) {
    entity_name = media_entity_get_devname(entity);
    if (entity_name) {
  ...
  entity = media_get_entity_by_name_rk(device, "rkcif-dvp-sof");
  if (entity) {
    entity_name = media_entity_get_devname(entity);
    if (entity_name) {
...
int RKAiqMedia::GetMediaInfo() {
  ...
  while (index < MAX_MEDIA_NUM) {
    ...
    RKMEDIA_LOGI("RKAIQ: parsing sys_path %s\n", sys_path);
...
int RKAiqMedia::DumpMediaInfo() {
  ...
  for (int i = 0; i < MAX_CAM_NUM; i++) {
    ...
    if (isp->linked_sensor)
      RKMEDIA_LOGI("RKAIQ: \t link_isp_sensor_name :    %s\n", isp->sensor_name.c_str());
    else if (cif->linked_sensor)
      RKMEDIA_LOGI("RKAIQ: \t link_cif_sensor_name :    %s\n", cif->sensor_name.c_str());
    else if (cif_dvp->linked_sensor && i ==2)
      RKMEDIA_LOGI("RKAIQ: \t link_cif_dvp_sensor_name :    %s\n", cif_dvp->sensor_name.c_str());
    ...
    if (i == 2) {
      if (cif_dvp->model_idx >= 0) {
        RKMEDIA_LOGI("RKAIQ: #### cif_dvp:\n");
        RKMEDIA_LOGI("RKAIQ: \t model_idx :         %d\n", cif_dvp->model_idx);
        RKMEDIA_LOGI("RKAIQ: \t linked_sensor :     %d\n", cif_dvp->linked_sensor);
        RKMEDIA_LOGI("RKAIQ: \t media_dev_path :    %s\n",
                     cif_dvp->media_dev_path.c_str());
        RKMEDIA_LOGI("RKAIQ: \t dvp_id0 : 		  %s\n",
                     cif_dvp->dvp_id0.c_str());
        ...
        RKMEDIA_LOGI("RKAIQ: \t dvp_sd_path :      %s\n",
                     cif_dvp->dvp_sd_path.c_str());
        RKMEDIA_LOGI("RKAIQ: \t mipi_luma_path :    %s\n",
                     cif_dvp->mipi_luma_path.c_str());
        RKMEDIA_LOGI("RKAIQ: \t dvp_sof_path :    %s\n",
                     cif_dvp->dvp_sof_path.c_str());
      }
    }
    if (isp->model_idx >= 0) {
