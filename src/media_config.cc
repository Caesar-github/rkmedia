...
bool ParseMediaConfigFromMap(std::map<std::string, std::string> &params,
  ...
  if (!image_in && !video_in && !audio_in) {
    RKMEDIA_LOGE("%s: unsupport outtype %s\n", __func__, value.c_str());
    return false;
  }
  ImageInfo info;
  ...
  if (image_in || video_in) {
    ...
    GET_STRING_TO_INT(img_cfg.rect_info.x, params, KEY_RECT_X, 0);
    GET_STRING_TO_INT(img_cfg.rect_info.y, params, KEY_RECT_Y, 0);
  ...
  if (image_in) {
    ...
    // for pass "rotation" info frome video_config
    ...
    GET_STRING_TO_INT(vid_cfg.rotation, params, KEY_ROTATION, 0)
    GET_STRING_TO_INT(vid_cfg.scaling_list, params, KEY_SCALING_LIST, 0)
...
std::string to_param_string(const VideoConfig &vid_cfg) {
  ...
  PARAM_STRING_APPEND_TO(ret, KEY_SCALING_LIST, vid_cfg.scaling_list);
  return ret;
}
...
std::string get_video_encoder_config_string(const ImageInfo &info,
  ...
  if (cfg.fps)
    ...
    RKMEDIA_LOGI("%s: frame rate use defalut value:30\n", __func__);
  }
  ...
  if (cfg.max_bps) {
    ...
    RKMEDIA_LOGI("%s: maxbps use defalut value:%d\n", __func__,
